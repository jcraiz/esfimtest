...
const ESFIMAnswerService = (() => {
  ...
  /**
   * Convert an overall speaking CEFR level key into its percentage equivalent.
   * Mirrors ESFIMScoring.speakingPctForLevel (kept local so this file is
   * self-contained): A0=5%, A1=20%, A1+=30%, A2=40%, A2+=50%, B1=60%,
   * B1+=70%, B2=80%, B2+=90%, C1=100%. Unknown/empty → 0.
   */
  function speakingPctForLevel(level) {
    if (!level) return 0;
    const map = {
      'A0': 5, 'A1': 20, 'A1+': 30, 'A2': 40, 'A2+': 50,
      'B1': 60, 'B1+': 70, 'B2': 80, 'B2+': 90, 'C1': 100,
    };
  ...
  function computeAllScores(attempt, exam) {
    ...
    // v2.0b+ — component-based Overall Score (equal section weights):
    //   With speaking:    Overall = (Writing + Reading + Listening + Speaking)/4
    //   Without speaking: Overall = (Writing + Reading + Listening)/3
    // where Writing = (Vocab% + Grammar%)/2 counts as a single component.
    ...
    const comps = [writingScore, reading.pct, listening.pct];
    if (speakingIncluded) comps.push(speakingPct);
    const overall = comps.length > 0 ? Math.round(comps.reduce((a, b) => a + b, 0) / comps.length) : 0;
