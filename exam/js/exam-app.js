...
(() => {
  ...
  // v2.0b — automatic translation lock. While the ESFIMTranslationGuard shows
  // its blocking prompt (browser/device translation detected), the countdown
  // timer is paused so the candidate does not lose time fixing the issue.
  let translationLocked = false;

  document.addEventListener('esfim:exam-lock', () => { translationLocked = true; });
  document.addEventListener('esfim:exam-unlock', () => { translationLocked = false; });
  ...
  function renderExamShell() {
    ...
    // v2.0b — mark the attempt as in progress (translation guard stays active)
    window.__ESFIM_EXAM_STATE__ = 'in_progress';
  ...
  function startTimer() {
    ...
    timerInterval = setInterval(() => {
      // v2.0b — freeze the countdown while the translation guard is blocking.
  ...
  async function submitExam(reason = 'Completed') {
    ...
    // v2.0b — results are final; the translation guard no longer blocks the screen
    window.__ESFIM_EXAM_STATE__ = 'submitted';
    ...
    if (typeof ESFIMAnswerService !== 'undefined') {
      ...
      attempt.scores = ESFIMAnswerService.computeAllScores(attempt, exam);
    } else {
      attempt.scores = ESFIMScoring.computeAllScores(attempt, exam);
    }
    ESFIMAttemptStore.saveAttempt(attempt);
